...
void setup(){
 ...
 //Video setup
 //Video to simulate mirror environment
...
//Begins video loop sets background and forces the background image to be the video from the webcam
 if (video.available()) { // Checks if video is available (Camera)
   video.read(); //If camera read turn on
 }
 background(0);
 image(video,0,0);
 ...
 // Year is a built in Processing function that communicates with the clock on the computer and returns the current year as an integer.
  ...
  fill(0,0,255);
  text(month(),1200,50);
  if (day() < 10){ //Add a leading zero if the day is less than the 10th of the month
    text("0", 1240,50);
  }
  text("/", 1260,50);
  text(day(), 1280,50);
 ...
 // Seconds is a built in Processing function that communicates with the clock on the computer and returns the current second as a value from 0 - 59.
  ...
  fill(0,255,0);
  if (hour() > 12){ //If statement used to convert military time to standard time
    text(hour()-12, 1500, 50);
  }
  else {
    text(hour(), 1500, 50);
  }
  text(":", 1570, 50);
  if (minute() < 10){ //If statement to add leading zero if less than 10mins into the hour
    text("0", 1590,50);
  }
  text(minute(), 1600,50);
  text(":", 1665,50);
